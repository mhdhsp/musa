// Ultra-Reliable Google Sheets Service with Triple-Redundant Webhook Sync & Test Suite
 ...
 * 7. COPY THE WEB APP URL and paste it into your application Settings!
 * ====================================================================
 */
...
function handleAllRequests(e) {
  ...
  try {
    ...
    if (reqAction === 'SAVE_STUDENT') {
      saveStudentRow(ss, data.student);
    } else if (reqAction === 'DELETE_STUDENT') {
      deleteStudentRow(ss, data.studentId);
    } else if (reqAction === 'SAVE_ATTENDANCE') {
      saveAttendanceRow(ss, data.attendance);
    } else if (reqAction === 'SAVE_HIFZ_LOG') {
      saveHifzLogRow(ss, data.hifzLog);
    } else if (reqAction === 'FULL_SYNC') {
      fullSync(ss, data.payload);
    } else if (reqAction === 'TEST_ROW') {
      saveStudentRow(ss, {
        id: 'TEST-999',
        rollNo: 'TEST-01',
        name: 'Connection Test Student',
        section: 'HIFZ',
        classLevel: 'Test Level',
    ...
    return createJsonResponse({ status: 'success', message: 'Action processed in Google Sheet!' });
  } catch (err) {
...
function saveStudentRow(ss, student) {
  ...
  var row = [
    student.id, student.rollNo, student.name, student.section, student.classLevel || '',
    student.guardianName || '', student.phone || '', student.joinedDate || '',
    student.status || 'active', student.totalJuzMemorized || 0, student.currentJuz || 1, student.currentSurah || ''
  ];
  if (rowIndex > 0) {
...
function saveHifzLogRow(ss, log) {
  ...
  sheet.appendRow([
    log.id, log.studentId, log.studentName, log.date, log.sabahSurah, log.sabahAyahs,
    log.sabahGrade, log.sabqiJuz, log.sabqiGrade, log.manzilJuz, log.manzilGrade, log.teacherRemarks
  ]);
}
...
function fullSync(ss, payload) {
  ...
  // Clear and rewrite Students sheet
  if (payload.students) {
  ...
  if (payload.hifzLogs && payload.hifzLogs.length > 0) {
...
export async function testGoogleSheetConnection(webAppUrl) {
  ...
  try {
    ...
    return {
      success: true,
      message: "Connection Successful! A 'Connection Test Student' row was sent to your Google Sheet."
    };
  } catch (err) {
    return {
      success: false,
      message: "Could not reach Google Sheet. Please verify your Web App URL."
    };
  }
}
...
export async function sendToGoogleSheet(webAppUrl, action, data) {
  ...
  try {
    ...
    const getUrl = `${cleanUrl}?action=${action}&data=${encodeURIComponent(jsonString)}`;
...
function submitViaHiddenForm(url, payload) {
  try {
    ...
    console.warn("Hidden form submit fallback notice:", e);
  }
}
